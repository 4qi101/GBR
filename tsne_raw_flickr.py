...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--dset', type=str, default='coco',
                        ...
                        help='dataset name')
    parser.add_argument('--output_dir', type=str, default='raw_tsne_figs_strict_single_label_coco',
    ...
    parser.add_argument('--top_k', type=int, default=0, help='Top K classes.')
...
def plot_embedding(emb: np.ndarray, labels: np.ndarray, title: str, out_path: str,
    ...
    if color_by_label:
        ...
        cbar = plt.colorbar(scatter, ticks=np.arange(num_classes), label='Class ID')
        cbar.ax.set_yticklabels(unique_labels)
    else:
        plt.scatter(emb[:, 0], emb[:, 1], c=fallback_color, s=20, alpha=0.6)

    if annotate_clusters:
        labels_use_largest_cluster = labels_use_largest_cluster or set()
        for lbl in unique_labels:
            mask = labels == lbl
            if not np.any(mask):
                continue
            if lbl in labels_use_largest_cluster:
                center = largest_cluster_center(emb[mask])
            else:
                center = emb[mask].mean(axis=0)
            plt.text(center[0], center[1], str(int(lbl)), fontsize=8, fontweight='bold',
